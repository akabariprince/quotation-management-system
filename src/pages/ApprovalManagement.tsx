...
import {
  Check, X, Clock, Shield, ShieldCheck, Search, RefreshCw,
  AlertCircle, KeyRound, Loader2, ChevronLeft, ChevronRight,
  Send, FileText, ArrowLeft,
} from "lucide-react";
...
// ─── Types ───
interface OTPLog {
  id: string; type: "login" | "discount" | "master_activation";
  entityId: string | null; entityType: string | null; entityName: string | null;
  email: string; status: "pending" | "approved" | "expired";
  attempts: number; maxAttempts: number; expiresAt: string;
...
interface Stats { pending: number; approvedToday: number; expiredToday: number; total: number; }
  ...
  <div className="enterprise-card overflow-hidden">
    <div className="table-container">
      <table className="enterprise-table">
        <thead><tr>{Array.from({ length: 7 }).map((_, i) => (<th key={i} className="px-3 py-1.5"><Skeleton className="h-3 w-16" /></th>))}</tr></thead>
        <tbody>{Array.from({ length: rows }).map((_, i) => (
          <tr key={i}>{Array.from({ length: 7 }).map((_, j) => (<td key={j} className="px-3 py-1"><Skeleton className="h-3 w-full" /></td>))}</tr>
        ))}</tbody>
      </table>
    </div>
  </div>
...
const OTPVerifyModal: React.FC<{
  isOpen: boolean; onClose: () => void; onVerify: (otp: string) => void;
  ...
  React.useEffect(() => {
    if (isOpen) { setOtp(""); setTimeout(() => inputRefs.current[0]?.focus(), 100); }
  }, [isOpen]);
        ...
        <div className="flex justify-center gap-2 mb-4" onPaste={handlePaste}>
          {Array.from({ length: 6 }).map((_, i) => (
            <input key={i} ref={(el) => { inputRefs.current[i] = el; }}
              ...
              className="otp-input" disabled={loading} />
          ))}
        </div>
        <div className="flex gap-2">
          <Button variant="outline" className="flex-1 h-7 text-xs" onClick={onClose} disabled={loading}>Cancel</Button>
          <Button className="flex-1 btn-accent h-7 text-xs" onClick={() => onVerify(otp)} disabled={otp.length !== 6 || loading}>
            {loading ? (<><Loader2 className="h-3 w-3 animate-spin mr-1" />Verifying...</>) : (<><Check className="h-3 w-3 mr-1" />Verify & Approve</>)}
          </Button>
        </div>
      </div>
    </div>
...
const RejectModal: React.FC<{
  isOpen: boolean; onClose: () => void; onReject: (reason: string) => void;
  loading: boolean; itemName: string;
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-content max-w-md p-4" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4">
          ...
          <p className="text-xs text-muted-foreground text-center mt-1">Reject <strong>{itemName}</strong>?</p>
        </div>
        <div className="space-y-1.5 mb-4">
          <label className="text-xs font-medium">Reason (optional)</label>
          <textarea value={reason} onChange={(e) => setReason(e.target.value)}
            placeholder="Provide a reason for rejection..."
            className="input-field min-h-[60px] resize-none text-xs" disabled={loading} />
        </div>
        <div className="flex gap-2">
          <Button variant="outline" className="flex-1 h-7 text-xs" onClick={onClose} disabled={loading}>Cancel</Button>
          <Button variant="destructive" className="flex-1 h-7 text-xs" onClick={() => onReject(reason)} disabled={loading}>
            {loading ? (<><Loader2 className="h-3 w-3 animate-spin mr-1" />Rejecting...</>) : (<><X className="h-3 w-3 mr-1" />Reject</>)}
          </Button>
        </div>
      </div>
    </div>
...
const ApprovalManagement: React.FC = () => {
  ...
  const [rejectModal, setRejectModal] = useState<{ open: boolean; item: OTPLog | null }>({ open: false, item: null });

  const [confirmDialog, setConfirmDialog] = useState<{
    open: boolean; title: string; description: string;
    onConfirm: () => void; variant: "danger" | "warning" | "info";
    loading: boolean; confirmText: string;
  }>({ open: false, title: "", description: "", onConfirm: () => { }, variant: "info", loading: false, confirmText: "Approve" });

  // ─── Fetch Data ───
  const fetchStats = useCallback(async () => {
    try { const res = await api.get("/otp-logs/stats"); if (res.success) setStats(res.data); } catch (err) { console.error("Failed to fetch stats:", err); }
  }, []);

  const fetchPending = useCallback(async () => {
    try {
      const res = await api.get(`/otp-logs/pending?page=${pendingPage}&limit=10`);
      if (res.success) { setPendingLogs(res.data || []); setPendingMeta(res.meta || null); }
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      if (res.success) { setAllLogs(res.data || []); setAllMeta(res.meta || null); }
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimerRef.current = setTimeout(() => { setAllPage(1); fetchAll(); }, 400);
  };
  ...
  const handleDirectApprove = (log: OTPLog) => {
    ...
    setConfirmDialog({
      ...
      variant: "info", loading: false, confirmText: "Approve Now",
      onConfirm: async () => {
        setConfirmDialog((prev) => ({ ...prev, loading: true }));
        try {
          const res = await api.post(`/otp-logs/${log.id}/direct-approve`, {});
          if (res.success) { toast.success(`"${entityLabel}" approved and activated successfully`); await refreshAll(); }
          else toast.error(res.message || "Failed to approve");
        } catch (err: any) { toast.error(err?.message || "Failed to approve directly"); }
  ...
  const handleBatchDirectApprove = () => {
    const nonExpiredLogs = pendingLogs.filter((log) => !isExpired(log.expiresAt));
    if (nonExpiredLogs.length === 0) { toast.info("No active pending items to approve"); return; }
    setConfirmDialog({
      open: true, title: "Approve All Pending",
      ...
      variant: "warning", loading: false, confirmText: `Approve All (${nonExpiredLogs.length})`,
      onConfirm: async () => {
        setConfirmDialog((prev) => ({ ...prev, loading: true }));
        let successCount = 0; let failCount = 0;
        for (const log of nonExpiredLogs) {
          try { const res = await api.post(`/otp-logs/${log.id}/direct-approve`, {}); if (res.success) successCount++; else failCount++; }
          catch { failCount++; }
        }
        if (successCount > 0) toast.success(`${successCount} item(s) approved successfully${failCount > 0 ? `, ${failCount} failed` : ""}`);
        if (failCount > 0 && successCount === 0) toast.error("Failed to approve items");
        await refreshAll();
  ...
  const handleApprove = async (otp: string) => {
    ...
    try {
      const res = await api.post(`/otp-logs/${otpModal.item.id}/approve`, { otp });
      if (res.success) { toast.success("Approved successfully"); setOtpModal({ open: false, item: null }); await refreshAll(); }
  ...
  const handleReject = async (reason: string) => {
    ...
    setActionLoading(true);
    try {
      const res = await api.post(`/otp-logs/${rejectModal.item.id}/reject`, { reason });
      if (res.success) { toast.success("Rejected successfully"); setRejectModal({ open: false, item: null }); await refreshAll(); }
  ...
  const getEntityDisplayName = (log: OTPLog): string => { if (log?.entityName) return log.entityName; if (log?.entityType) return log.entityType; return getTypeLabel(log?.type); };
  const getRequesterName = (log: OTPLog): string => { if (log.requester?.name) return log.requester.name; if (log.requester?.email) return log.requester.email; if (log.email) return log.email; return "—"; };
  const getApproverName = (log: OTPLog): string => { if (log.approver?.name) return log.approver.name; if (log.approver?.email) return log.approver.email; return "—"; };

  const pendingCount = stats?.pending ?? pendingMeta?.totalCount ?? pendingMeta?.totalItems ?? 0;
  const totalCount = stats?.total ?? allMeta?.totalCount ?? allMeta?.totalItems ?? 0;
  const approvedToday = stats?.approvedToday ?? 0;
  const expiredToday = stats?.expiredToday ?? 0;
  const nonExpiredPendingCount = pendingLogs.filter((log) => !isExpired(log.expiresAt)).length;
    ...
    return (
      <div className="animate-fade-in">
        <div className="flex items-center justify-between py-1">
          <div><Skeleton className="h-4 w-40" /><Skeleton className="h-3 w-56 mt-1" /></div>
        </div>
        <StatsSkeleton />
        <div className="mt-1"><TableSkeletonComponent rows={5} /></div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between py-1">
        <div>
          <h1 className="text-sm font-semibold leading-none">Approval Management</h1>
          <p className="text-muted-foreground text-xs">Review and approve pending OTP requests</p>
        </div>
        <div className="flex items-center gap-1">
          ...
          {isAdmin && hasPermission("approval:manage") && nonExpiredPendingCount > 0 && (
            <Button size="sm" className="gap-1 h-7 text-xs px-2 bg-green-600 hover:bg-green-700 text-white" onClick={handleBatchDirectApprove}>
              <ShieldCheck className="h-3 w-3" />
              <span className="hidden sm:inline text-white">Approve All</span>
              <span className="bg-white/20 px-1 py-px rounded text-xs">{nonExpiredPendingCount}</span>
            </Button>
          )}
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-2 mt-2">
        <div className="overflow-x-auto display flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="gap-1 h-7 text-xs px-2 ml-1" onClick={handleRefresh}>
        ...
        <TabsContent value="pending" className="space-y-1">
          <div className="enterprise-card overflow-hidden">
              ...
              <table className="enterprise-table w-full">
                <thead>
                  <tr>
                    ...
                    <th className="hidden md:table-cell px-3 py-1.5 text-xs">Email</th>
                    <th className="hidden lg:table-cell px-3 py-1.5 text-xs">Expires</th>
                ...
                <tbody>
                  {pendingLogs.length === 0 ? (
                    ...
                    pendingLogs.map((log) => {
                      ...
                      return (
                        <tr key={log.id} className={expired ? "opacity-60" : ""}>
                          <td className="px-3 py-1"><span className={getTypeBadge(log?.type)}>{getTypeLabel(log?.type)}</span></td>
                          <td className="px-3 py-1 text-sm">
                            {log?.entityType ? (
                              <div className="flex flex-col gap-0.5">
                                <span className="bg-muted px-1.5 py-px rounded text-xs w-fit capitalize">{log.entityType}</span>
                                {log?.entityName && (<span className="text-xs font-medium text-foreground truncate max-w-[140px]" title={log.entityName}>{log.entityName}</span>)}
                              </div>
                            ) : (<span className="text-muted-foreground">—</span>)}
                          </td>
                          <td className="hidden sm:table-cell px-3 py-1 text-sm">
                            <span className="font-medium text-foreground">{getRequesterName(log)}</span>
                          </td>
                          <td className="hidden md:table-cell px-3 py-1 text-muted-foreground text-sm">{log.email}</td>
                          <td className="hidden lg:table-cell px-3 py-1 text-sm">
                            {expired ? (<span className="text-destructive font-medium">Expired</span>) : (<span className="text-muted-foreground">{formatDate(log.expiresAt)}</span>)}
                          </td>
                          <td className="px-3 py-1"><span className={expired ? "badge-error" : "badge-warning"}>{expired ? "Expired" : "Pending"}</span></td>
                          <td className="px-3 py-1">
                            {!expired ? (
                              <div className="flex items-center gap-1 flex-wrap">
                                {hasPermission("approval:manage") && (
                                  isAdmin ? (
                                    <Button size="sm" className="h-6 text-xs px-2 bg-green-600 hover:bg-green-700 text-white gap-1" onClick={() => handleDirectApprove(log)}>
                                      <ShieldCheck className="h-3 w-3" />Approve
                                    </Button>
                                  ) : (
                                    <Button size="sm" className="h-6 text-xs px-2 bg-success hover:bg-success/90 text-success-foreground" onClick={() => setOtpModal({ open: true, item: log })}>
                                      <Check className="h-3 w-3 mr-0.5" />Approve
                                    </Button>
                                  )
                                )}
                                {hasPermission("approval:manage") && (
                                  <Button size="sm" variant="outline" className="h-6 text-xs px-2 text-destructive border-destructive/50 hover:bg-destructive/10"
                                    onClick={() => setRejectModal({ open: true, item: log })}>
                                    <X className="h-3 w-3 mr-0.5" />Reject
                                  </Button>
                                )}
                                {hasPermission("approval:manage") && !isAdmin && (
                                  <Button size="sm" variant="ghost" className="h-6 text-xs px-1.5" onClick={() => handleResend(log)} title="Resend OTP">
            ...
            {pendingMeta && pendingMeta.totalPages > 1 && (
              <div className="flex items-center justify-between px-3 py-1.5 border-t border-border">
                <p className="text-xs text-muted-foreground">Page {pendingMeta.currentPage} of {pendingMeta.totalPages}</p>
                <div className="flex gap-1">
                  <Button size="sm" variant="outline" className="h-6 text-xs px-2" disabled={pendingPage <= 1} onClick={() => setPendingPage((p) => p - 1)}>
                    <ChevronLeft className="h-3.5 w-3.5" />
                  </Button>
                  <Button size="sm" variant="outline" className="h-6 text-xs px-2" disabled={pendingPage >= pendingMeta.totalPages} onClick={() => setPendingPage((p) => p + 1)}>
                    <ChevronRight className="h-3.5 w-3.5" />
                  </Button>
                </div>
              </div>
        ...
        <TabsContent value="all" className="space-y-1">
          <div className="enterprise-card p-2">
            <div className="flex flex-col sm:flex-row items-center gap-1.5">
              <div className="relative flex-1 w-full">
                ...
                <Input placeholder="Search by email, entity name..." value={searchTerm}
                  onChange={(e) => handleSearchChange(e.target.value)} className="pl-7 h-7 text-xs" />
              </div>
              <Select value={filterType} onValueChange={(v) => { setFilterType(v); setAllPage(1); }}>
                <SelectTrigger className="w-full sm:w-[120px] h-7 text-xs px-2"><SelectValue placeholder="Type" /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={filterStatus} onValueChange={(v) => { setFilterStatus(v); setAllPage(1); }}>
          ...
          <div className="enterprise-card overflow-hidden">
              ...
              <table className="enterprise-table w-full">
                <thead>
                  <tr>
                    ...
                    <th className="px-3 py-1.5 text-xs">Type</th>
                    <th className="hidden sm:table-cell px-3 py-1.5 text-xs">Entity</th>
                    <th className="hidden md:table-cell px-3 py-1.5 text-xs">Requested By</th>
                    <th className="hidden lg:table-cell px-3 py-1.5 text-xs">Approved By</th>
                    <th className="px-3 py-1.5 text-xs">Status</th>
                    {isAdmin && <th className="px-3 py-1.5 text-xs">Actions</th>}
                  </tr>
                </thead>
                <tbody>
                  {allLogs.length === 0 ? (
                    <tr><td colSpan={isAdmin ? 7 : 6} className="text-center text-muted-foreground py-8 text-sm">
                      <FileText className="h-6 w-6 mx-auto mb-1 opacity-50" />No OTP logs found.
                    </td></tr>
                  ) : (
                    allLogs.map((log) => (
                      <tr key={log.id}>
                        <td className="px-3 py-1 text-muted-foreground text-sm whitespace-nowrap">{formatDate(log.createdAt)}</td>
                        <td className="px-3 py-1"><span className={getTypeBadge(log?.type)}>{getTypeLabel(log?.type)}</span></td>
                        <td className="hidden sm:table-cell px-3 py-1 text-sm">
                          {log?.entityType ? (
                            <div className="flex flex-col gap-0.5">
                              <span className="bg-muted px-1.5 py-px rounded text-xs w-fit capitalize">{log.entityType}</span>
                              {log?.entityName && (<span className="text-xs text-foreground font-medium truncate max-w-[120px]" title={log.entityName}>{log.entityName}</span>)}
                            </div>
                        ...
                        <td className="hidden lg:table-cell px-3 py-1 text-sm">
                          {log.status === "approved" ? (
                            <div className="flex flex-col">
                              <span className="font-medium text-foreground">{getApproverName(log)}</span>
                              {log.approvedAt && (<span className="text-xs text-muted-foreground">{formatDate(log.approvedAt)}</span>)}
                            </div>
                          ) : (<span className="text-muted-foreground">—</span>)}
                        </td>
                        <td className="px-3 py-1">
                          <span className={getStatusBadge(log.status)}>{log.status.charAt(0).toUpperCase() + log.status.slice(1)}</span>
                        </td>
                        {isAdmin && (
                          <td className="px-3 py-1">
                            {log.status === "pending" && !isExpired(log.expiresAt) ? (
                              <div className="flex items-center gap-0.5">
                                <Button size="sm" variant="outline" className="h-6 text-xs gap-0.5 px-1.5 text-green-700 border-green-300 hover:bg-green-50 dark:text-green-400 dark:border-green-700 dark:hover:bg-green-950/30"
                                  onClick={() => handleDirectApprove(log)}>
                                  <ShieldCheck className="h-3 w-3" />Approve
                                </Button>
                                <Button size="sm" variant="outline" className="h-6 text-xs gap-0.5 px-1.5 text-destructive border-destructive/30 hover:bg-destructive/5"
                                  onClick={() => setRejectModal({ open: true, item: log })}>
                                  <X className="h-3 w-3" />Reject
                                </Button>
                              </div>
            ...
            {allMeta && allMeta.totalPages > 1 && (
              <div className="flex items-center justify-between px-3 py-1.5 border-t border-border">
                <p className="text-xs text-muted-foreground">
                  Showing {allLogs.length} of {allMeta.totalCount || allMeta.totalItems} • Page {allMeta.currentPage} of {allMeta.totalPages}
                </p>
                <div className="flex gap-1">
                  <Button size="sm" variant="outline" className="h-6 text-xs px-2" disabled={allPage <= 1} onClick={() => setAllPage((p) => p - 1)}>
                    <ChevronLeft className="h-3.5 w-3.5" />
                  </Button>
                  <Button size="sm" variant="outline" className="h-6 text-xs px-2" disabled={allPage >= allMeta.totalPages} onClick={() => setAllPage((p) => p + 1)}>
                    <ChevronRight className="h-3.5 w-3.5" />
                  </Button>
                </div>
              </div>
      ...
      <OTPVerifyModal isOpen={otpModal.open} onClose={() => setOtpModal({ open: false, item: null })}
        onVerify={handleApprove} loading={actionLoading}
        title={`Approve ${getEntityDisplayName(otpModal.item!)}`}
        description="Enter the OTP to verify and approve this request" />

      <RejectModal isOpen={rejectModal.open} onClose={() => setRejectModal({ open: false, item: null })}
        onReject={handleReject} loading={actionLoading} itemName={getEntityDisplayName(rejectModal.item!)} />
      ...
      <ConfirmDialog open={confirmDialog.open}
        ...
        loading={confirmDialog.loading} confirmText={confirmDialog.confirmText} cancelText="Cancel" />
    </div>
  );
};
